...
public class IndexController {
    ...
    public HttpEntity<byte[]> image(@ModelAttribute("path") String path, @ModelAttribute("size") String size, Model model) {
        File file = new File(folder + File.separator + path);
        byte[] byteImg = null;
        HttpHeaders httpHeaders = null;
        try {
            File file1 = size != null && size.equals("thumbnail") ? getNoResizeFile(file).orElse(file) : file;
            byteImg = Files.readAllBytes(file1.toPath());
